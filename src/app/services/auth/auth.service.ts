...
export class AuthService{
  ...
  login(username: string, password: string): Observable<User[] | undefined>{
    return this.http.get<User[]>(`${this.authApiUrl}?username=${username}&password=${password}`).pipe(
      tap(users => {
        const user = users[0];
        if(!user || !user.token){
          this.toastr.error("Connexion échouée :  Nom d'utilisateur ou mot de passe incorrect.", 'Erreur');
        ...
        this.toastr.success("Connexion réussi", 'Succès');
      })
    );
  }
  ...
  logout(): void{
    ...
    this.toastr.success("Déconnexion réussi", 'Succès');
  }
